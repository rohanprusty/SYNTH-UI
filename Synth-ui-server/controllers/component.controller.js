...
export const publishComponent = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.role !== "admin") {
      log("ERROR: Only admin can publish");
      return res.end();
    }
    ...
    if (component.owner.toString() !== req.userId.toString()) {
      log("ERROR: You can only publish your own components");
      return res.end();
    }

    const npmToken = process.env.NPM_TOKEN;
    if (!npmToken) {
      log("INFO: NPM_TOKEN environment variable is not set. Relying on global ~/.npmrc for authentication.");
    }

    log("Building package...");

    // 1. Generate temp directory
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "synth-ui-publish-"));
    log(`Created temporary directory: ${tempDir}`);

    const pkgName = `@hustlerx99/synth-ui-${component.name.toLowerCase()}`;
    
    // 2. Determine version
    let currentVersion = "1.0.0";
    try {
      log("Checking current version from npm registry...");
      const { stdout } = await execa("npm", ["view", pkgName, "version"]);
      if (stdout) {
        currentVersion = semver.inc(stdout.trim(), "patch");
        log(`Incremented version to ${currentVersion}`);
      }
    } catch (e) {
      log(`Package not found or error, starting at version ${currentVersion}`);
    }
    ...
    const packageJson = {
      ...
      description: `AI generated component: ${component.name}`,
      main: "dist/index.js",
      type: "module",
      peerDependencies: {
        "react": "^18.0.0",
    ...
    fsExtra.writeJsonSync(path.join(tempDir, "package.json"), packageJson, { spaces: 2 });
    log("Generated package.json");

    // 4. Generate README.md
    const readmeContent = `# ${component.name}\n\nAI generated component for Synth-UI.\n\n## Usage\n\n\`\`\`jsx\nimport { ${component.name} } from '${pkgName}';\n\nexport default function App() {\n  return <${component.name} />;\n}\n\`\`\``;
    fsExtra.writeFileSync(path.join(tempDir, "README.md"), readmeContent);
    log("Generated README.md");

    // 5. Generate .npmrc (if token provided)
    if (npmToken) {
      const npmrcContent = `//registry.npmjs.org/:_authToken=${npmToken}\n`;
      fsExtra.writeFileSync(path.join(tempDir, ".npmrc"), npmrcContent);
      log("Generated .npmrc securely using NPM_TOKEN");
    } else {
      log("Skipped generating .npmrc (using system default credentials)");
    }

    // 6. Copy component files
    const distDir = path.join(tempDir, "dist");
    fsExtra.ensureDirSync(distDir);
    fsExtra.writeFileSync(path.join(distDir, "index.js"), component.code);
    log(`Copied component files to dist/`);

    // 7. Publish
    log("Publishing...");
    
    const publishProcess = execa("npm", ["publish", "--access", "public", "--registry=https://registry.npmjs.org/"], {
      cwd: tempDir,
      reject: false
    });

    publishProcess.stdout.on("data", (data) => {
      res.write(`NPM: ${data.toString()}`);
    });

    publishProcess.stderr.on("data", (data) => {
      res.write(`NPM ERROR: ${data.toString()}`);
    });

    const result = await publishProcess;

    if (result.exitCode === 0) {
      log(`Success Logs: Published ${pkgName}@${currentVersion} successfully.`);
      
      component.visibility = "public";
      component.npmPackage = pkgName;
      await component.save();

      log(`URL: https://www.npmjs.com/package/${pkgName}`);
    } else {
      log(`ERROR: Publish failed with exit code ${result.exitCode}`);
    }
