  ...
  const handlePublish = async () => {
    ...
    try {
      const response = await fetch(`${ServerUrl}/api/component/publish`, {
        ...
        body: JSON.stringify({ componentId: savedComponentId }),
        credentials: "include",
      });
  ...
  <LiveComponentPreview code={generated.code} />
              ...
              {(publishLogs.length > 0 || publishing) && (
                <div className="px-5 pb-5">
                  <div className="bg-[#0d1117] rounded-xl p-4 border border-white/10 font-mono text-xs overflow-y-auto max-h-48 flex flex-col gap-1.5">
                    {publishLogs.map((log, idx) => (
                      <span key={idx} className={
                        log.startsWith("ERROR:") ? "text-red-400" :
                        log.startsWith("Success") || log.startsWith("URL:") ? "text-green-400" :
                        "text-white/70"
                      }>
                        {log}
                      </span>
