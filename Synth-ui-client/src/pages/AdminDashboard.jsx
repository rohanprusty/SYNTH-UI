...
function AddComponentForm({ onSaved }) {
  ...
  const handlePublish = async () => {
    ...
    setPublishLogs([]);
    try {
      const response = await fetch(`${ServerUrl}/api/component/publish`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ componentId: savedId }),
        credentials: "include",
      });
      ...
      while (true) {
        ...
        setPublishLogs((prev) => [...prev, ...lines]);

        // Check for success or error
        const urlLog = lines.find((l) => l.startsWith("URL:"));
        if (urlLog) {
          setIsPublished(true);
          showToast("Published to npm successfully!", "success");
        }

        const errLog = lines.find((l) => l.startsWith("ERROR:"));
        if (errLog) {
          showToast(errLog.replace("ERROR: ", ""), "error");
        }
      }
    } catch (err) {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-5 sm:py-6 max-w-3xl w-full mx-auto">
      ...
      <div className="space-y-4 sm:space-y-5">
        ...
        {/* ── Publish Logs ── */}
        {(publishLogs.length > 0 || publishing) && (
          <div className="bg-[#0d1117] rounded-xl p-4 border border-white/10 font-mono text-xs overflow-y-auto max-h-48 flex flex-col gap-1.5 mt-2">
            {publishLogs.map((log, idx) => (
              <span key={idx} className={
                log.startsWith("ERROR:") ? "text-red-400" :
                log.startsWith("Success") || log.startsWith("URL:") ? "text-[#3be8ff]" :
                "text-white/70"
              }>
                {log}
              </span>
            ...
            {publishing && (
              <span className="text-[#3be8ff] animate-pulse flex items-center gap-2">
